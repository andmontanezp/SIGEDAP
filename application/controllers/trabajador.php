...
class Trabajador extends CI_Controller {
   ...
   public function search(){
      ...
      if (isset($_POST["cedula"])) {
         $data['query'] = $this->Model_Trabajador->getFilteredPagination($pagina, $porpagina, 'limit', 'tra_cedula', $this->input->post('cedula'), false);
         $cuantos = $this->Model_Trabajador->getFilteredPagination($pagina, $porpagina, 'cuantos', 'tra_cedula', $this->input->post('cedula'), false); 
      } else if (isset($_POST["nombres"])) {
         $data['query'] = $this->Model_Trabajador->getFilteredPagination($pagina, $porpagina, 'limit', 'concat(tra_nombres, " ", tra_apellidos)', $this->input->post('nombres'), false);
         $cuantos = $this->Model_Trabajador->getFilteredPagination($pagina, $porpagina, 'cuantos', 'concat(tra_nombres, " ", tra_apellidos)', $this->input->post('nombres'), false); 
      } else if (isset($_POST["gerencia"])) {
         $data['query'] = $this->Model_Trabajador->getFilteredPagination($pagina, $porpagina, 'limit', 'g.ger_id', $this->input->post('gerencia'), true);
         $cuantos = $this->Model_Trabajador->getFilteredPagination($pagina, $porpagina, 'cuantos', 'g.ger_id', $this->input->post('gerencia'), true); 
      } else if (isset($_POST["coordinacion"])) {
         $data['query'] = $this->Model_Trabajador->getFilteredPagination($pagina, $porpagina, 'limit', 'coo.coo_id', $this->input->post('coordinacion'), true);
         $cuantos = $this->Model_Trabajador->getFilteredPagination($pagina, $porpagina, 'cuantos', 'coo.coo_id', $this->input->post('coordinacion'), true); 
      } else if (isset($_POST["cargo"])) {
         $data['query'] = $this->Model_Trabajador->getFilteredPagination($pagina, $porpagina, 'limit', 'c.car_id', $this->input->post('cargo'), true);
         $cuantos = $this->Model_Trabajador->getFilteredPagination($pagina, $porpagina, 'cuantos', 'c.car_id', $this->input->post('cargo'), true); 
      }
   ...
   /* 
     ...
     Changes : Creation of a new schema for a document generation
     Date : 22-03-2015
   */

   public function PdfPrueba($trabajador_id) {
      
      date_default_timezone_set('America/Caracas');
      
      $this->load->model("Model_Document");

      $document = $this->Model_Document->find(1);
      $trabajador = $this->Model_Trabajador->find(1);
      
      $symbolTable = array(
         "image" => base_url('js/webcamjs/'.$trabajador->foto),
         "image.width" => "200",
         "image.height" => "100",
         "date" => date("d-m-Y"),
         "fullname" => $trabajador->tra_nombres." ".$trabajador->tra_apellidos,
         "nacionality" => $trabajador->nacionalidad,
         "documentid" => $trabajador->tra_cedula,
         "empresa" => "Supra Caracas C.A"
      );
      ...
      $header = '{DATE j-m-Y}||Empleado: ';
      $body = '';
      $body .= $this->replaceChars($document->doc_body, $symbolTable, "{{", "}}");
      
      $footer = '|Página {PAGENO} de {nb}|SUPRA Caracas C.A.';

      $pdf = $this->prepareDocument($pdf, $header, $footer);
   ...
   public function prepareDocument($document, $header, $footer) {
      $document->mirrorMargins = 1; // Use different Odd/Even headers and footers and mirror margins
      $document->defaultheaderfontsize = 10; /* in pts */
      $document->defaultheaderfontstyle = B; /* blank, B, I, or BI */
      $document->defaultheaderline = 1; /* 1 to include line below header/above footer */
      $document->defaultfooterfontsize = 12; /* in pts */
      $document->defaultfooterfontstyle = B; /* blank, B, I, or BI */
      $document->defaultfooterline = 1; /* 1 to include line below header/above footer */
      $document->SetHeader($header.$trabajador->tra_nombres." ".$trabajador->tra_apellidos);
      $document->SetFooter($footer);

      return $document;
   }

   public function replaceChars($string, $symbolTable, $beginIdentifier, $endIdentifier) {

      foreach ($symbolTable as $symbol => $value) {
         $string = str_replace($beginIdentifier.$symbol.$endIdentifier, $value, $string);
      }
